...
#
# 2026-08-26 수정 배경 (docs/qa_retrieval_part_plan.md "0-17" 참고): 사용자가
# "PollCard가 호출하는 함수 알려줘"를 물었는데 location으로 잘못 분류돼서
# (calls_forward 대신 shallow_neighborhood가 실행되는 바람에) COMMIT/CLASS
# 노드가 섞인 채 서로 안 이어진 지저분한 그래프가 나온 문제가 있었음. "OO가
# 호출하는 함수가 뭐야"류 질문이 flow의 "실행 순서" 예시 하나만 보고는
# location의 "역할을 묻는 질문"과 헷갈릴 수 있어서, flow 쪽에 이 패턴을
# 명시적으로 추가하고, 애매할 때의 기본값도 "location으로" → "flow와
# location이 헷갈리면 '무엇을 호출하는지'가 핵심이면 flow"로 더 구체화함.
QUESTION_CLASSIFICATION_PROMPT = """당신은 코드베이스에 대한 사용자 질문을 아래 4가지 유형 중
정확히 하나로 분류하는 분류기입니다.

- flow: 특정 기능/요청이 처리되는 실행 순서, 호출 흐름을 묻는 질문. "OO가
  호출하는 함수/메서드가 뭐야", "OO는 어떤 API를 호출해?"처럼 어떤 대상이
  "무엇을 호출하는지" 묻는 질문도 flow입니다 — 목록으로 물어봐도 결국
  호출 관계를 묻는 것이므로 location이 아닙니다.
  (예: "회원가입 요청이 들어오면 어떤 순서로 처리돼?",
       "PollCard가 호출하는 함수 알려줘")
...
- location: 특정 기능/API가 코드 어디에 위치하는지, 어떤 역할을 하는지
  묻는 질문. "무엇을 호출하는지"가 아니라 "어디에 있는지/무슨 역할인지"가
  핵심일 때만 location입니다.
  (예: "로그인 처리하는 코드가 어디에 있어?")

사용자 질문을 읽고 위 4가지 중 가장 적합한 유형 하나를 question_kind로
선택하고, 왜 그렇게 판단했는지 한 문장으로 reason에 적으세요. flow와
location 사이에서 애매하면, 질문의 핵심이 "무엇을 호출/실행하는지"이면
flow를, "어디에 있는지/무슨 역할인지"이면 location을 선택하세요. 그래도
정말 애매하면 location을 선택하세요.
"""

# 후속 질문 재작성 프롬프트 (question_rewriter.py에서 사용, 2026-08-26 신규)
# 세션에 이전 대화가 있을 때만 호출됨 — 첫 질문은 아예 LLM을 안 부름.
QUESTION_REWRITE_SYSTEM_PROMPT = """당신은 대화 맥락을 참고해서 후속 질문을 독립적인 질문으로
다시 쓰는 도우미입니다.

규칙:
- 이전 대화(질문/답변)를 참고해서, "그거"/"거기"/"그 메서드"처럼 대명사로
  표현되거나 주어가 생략된 부분이 있으면 이전 대화에서 실제로 가리키는
  대상(메서드명, 클래스명, 파일 경로, API 경로 등)으로 명시적으로 채워
  넣으세요.
- 이전 대화와 무관하게 이미 완전히 독립적인 질문이면 그대로 두세요 — 억지로
  바꾸지 마세요.
- 질문의 의도(무엇을 묻는지)는 절대 바꾸지 마세요. 표현만 명확하게 다듬는
  것입니다.
- 이전 대화에 없는 새로운 사실이나 코드를 추측해서 넣지 마세요.
- rewritten_question에는 재작성된 질문 하나만, reason에는 왜 그렇게 바꿨는지
  (또는 왜 그대로 두었는지) 한 문장으로 적으세요.
"""

QUESTION_REWRITE_USER_PROMPT = """이전 대화:
{history}
...
RESPONSE_INTENT_INSTRUCTIONS = {
    ...
    QueryIntent.HISTORY: (
        ...
        "stated_intent로 작성하세요. pull_requests/issues의 제목·본문에 변경 "
        "이유나 목적이 명시된 경우도 stated_intent의 근거로 사용할 수 있습니다. "
        "PR이나 이슈가 연결됐다는 사실만으로 stated_intent로 분류하지 마세요. "
        "코드 변화만으로 추정한 이유는 inference로 구분하고, "
        "확인할 수 없는 이유는 uncertainties에 작성하세요. 각 변경 claim에는 해당 "
        "version, commit, pull_requests, issues의 evidence_id 중 제공된 ID만 연결하세요. "
