...
def build_graph():
    """10개 노드를 조립한 LangGraph를 컴파일해서 반환.

    흐름:
        START -> question_rewriter -> question_analyzer -> entity_resolver
            -> vector_retriever -> target_selector -> graph_retriever
            -> evidence_enricher -> evidence_fusion -> evidence_validator
    ...
    question_rewriter(2026-08-26 신규, docs/qa_retrieval_part_plan.md "0-14"
    참고)는 반드시 question_analyzer보다 먼저 실행해야 한다 — 후속 질문의
    유형 분류(flow/impact/intent/location)도 재작성된(맥락이 채워진) 질문
    기준으로 해야 정확하기 때문. 세션에 이전 대화가 없으면(첫 질문) 이
    노드는 LLM 호출 없이 바로 통과한다.
    """
    graph = StateGraph(QAState)

    graph.add_node("question_rewriter", question_rewriter.rewrite_follow_up_question)
    ...
    graph.add_edge("question_rewriter", "question_analyzer")
